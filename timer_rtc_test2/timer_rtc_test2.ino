...
void runtimeTest() {
  ...
  Serial.println("ms");
  Serial.print((float)duration / 10000.0);
  Serial.println("ms");
  while (1);
}
...
void computeTendency() {
  ...
  Serial.print(" ");
  float tendency;
  if(pressures[p0] > pressures[p1])
    tendency = pressures[p0] - pressures[p1]; // steigend
  else
    tendency = -1 * (pressures[p1] - pressures[p0]); // fallend
